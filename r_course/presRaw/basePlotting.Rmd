...
~1/25 of people have color blindness. In white males specifically this number raises to  above 1/10. Considering the unequal demographics in science that means you will have someone with a form of color blindness at most meetings.

People with color blindness most often struggle to differentiate between red/green or blue/yellow.

Packages exist that contain color blind friendly pallettes like [viridis](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html). 

There are many packages that contain color pallettes in them. To get colors you just have to call the function with the number of colors you want. 
...
Let's read the data from the example.txt data file.  
...
Base graphics has a useful built in function for dotcharts too. The **dotchart()** function. This enables direct comparison between paired data.First though we need to modify the matrix, as in dotplots we are comparing each paired data set versus each other as opposed to control versus treatment. 

Here we use the function **t** to return the transpose of a matrix. This means rows are now columns and the columns are now rows. 

<!-- # ```{r dotchart_basePlotting,eval=FALSE} -->
<!-- # dotchart(t(data))	 -->
<!-- # ``` -->

```{r dotchartEval_basePlotting,fig.width=8,fig.height=4,dpi=300,out.width="1920px",height="1080px"}
dotchart(t(data))  
```
...
PDFs can then be opened in imaging software like illustrator or inkscape. This allows full customization of aesthetic with a graphic user interface. Furthermore as it is scalable plots can be easily assembled into publication quality figures, without pixelation. 
