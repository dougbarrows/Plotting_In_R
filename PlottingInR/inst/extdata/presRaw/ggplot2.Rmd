...
```{r, multiplegeom_ggplot2, fig.height=4.5, fig.width=8}
ggplot(data=patients_clean,
                 mapping=aes(x=Sex, y=Height))+ 
  geom_violin() +
  geom_jitter()
...
Another way to highlight differences between groups is with a marginal plot. Plots for either the X or Y axis variables can easily be added to the margins of the plot to display the distributions of variables. By default this is a line. The groupcolor and groupFill arguments carry over the color aesthetic of the main plot.  

```{r, marginal_ggplot, fig.height=4, fig.width=9, message = F, warning = F}
library(ggExtra)
pcPlot <- ggplot(data=patients_clean,
...
```{r, marginal_ggplot2, fig.height=4, fig.width=9, message = F, warning=F}
...
```{r, facet_grid_height_weight_brewerScale_ggplot2, fig.height=5, fig.width=9, message = F, warning = F}
pcPlot <- ggplot(data=patients_clean,
...
```{r, cale_color_paletteer_d, fig.height=4, fig.width=8, message = F, warning = F}
library(paletteer)
pcPlot <- ggplot(data=patients_clean,
                 aes(x=Height,y=Weight,color=Pet))
pcPlot + geom_point(size=4) + 
...
```{r, scale_color_viridis_d, fig.height=5, fig.width=9, message = F, warning = F}
...
![palettes](imgs/example_ggplot_defulats.jpg){width=75%}
...
```{r, stat_smooth_ggplot2, fig.height=4, fig.width=9}
pcPlot <- ggplot(data=patients_clean,
        mapping=aes(x=Height,y=Weight))
pcPlot+geom_point() + 
  stat_smooth()
...
```{r, line_eqn, fig.height=2.5, fig.width=7}
library(ggpubr)
...
```{r, eval=F, fig.height=3.5, fig.width=4,tidy=FALSE}
...
```{r, ggthemes, fig.height=3.5, fig.width=4,tidy=FALSE, echo=F}
library(ggthemes)
pcPlot+theme_wsj()
```
  ]
...
Lets look at some examples. We will look at two popular packages: clusterProfiler and Seurat.

---
## clusterProfiler

clusterProfiler is a package that is mostly used to check for enrichment of genes within data sets i.e. GO term analysis and GSEA. One of the main outputs is a dotplot that shows the top hits. We have an example here:
...
The dotplot function is a clusterProfiler specific function that is using ggplot2 to make a nice output plot. We can see the result of this is a ggplot object.
...
---
## Modifying clusterProfiler

As it is a ggplot object, we can customize it in the same way. Here we can change the colors and title. 
...
We created this using the DimPlot() function. Again this is specific to Seurat but it is using ggplot under the hood. 
...
As it is a ggplot object, we can customize it in the same way. Here we can change the theme, title and colors again (this time the scale is discrete). 
...
We can use the layout information to recreate the original ggplot, and then add some modifications i.e. add a size argument. 
